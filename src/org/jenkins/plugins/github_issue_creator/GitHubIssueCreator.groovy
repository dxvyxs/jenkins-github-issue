...
class CreationResult implements Serializable {
    ...
    boolean isSuccess() {
        return action in [CreationAction.CREATED, CreationAction.COMMENTED,
                          CreationAction.REOPENED, CreationAction.SKIPPED_CACHED]
    }
}
...
enum CreationAction {
    ...
    COMMENTED,      // Comment added to existing open issue
    REOPENED,       // Closed issue reopened + commented
    SKIPPED_CACHED, // Dedup cache hit (same build, already handled)
    SAFE_FAIL       // Operation failed gracefully
}
